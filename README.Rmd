...
brief outlook on the development roadmap.
...
each of them into a `for` loop was quite revealing, especially with the complexer functions, such as
`purrr::reduce()` when the direction is set to `"backward"`.
...
`as_loop()` can help with debugging. Usually a `for` loop will run over an index, for example `i`. 
When executed in the global environment, useRs can easily inspect the code in the console at index `i`
...
Now we can look at the documentation of `{purrr}` and start with translating the first example of
...
Credit goes to the creators and maintainers of the amazing `{purrr}` package! `{loopurrr}` is
just an add-on which would not exist without it.

Although this package contains tests with more than 1000 lines of code, there are definitely a
number of edge cases which don't work correctly. If you find one, I'd be happy if you file an
